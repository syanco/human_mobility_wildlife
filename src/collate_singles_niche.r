...
fl <- list.files("out/single_species_models/niche/")

for(i in 1:length(fl)){
  
  #load the model, species, and data
  load(glue("out/single_species_models/niche/{fl[i]}")) 
  ...
  raw_plot <- ggplot(data = out$data, aes(x = week, y = log_area, goup = grp, color = year_f)) +
  ...
  ggsave(filename = glue("out/single_species_models/niche_plots/{out$species}.pdf"), 
         plot = out_plot,
...
sink("out/single_species_models/niche_mods.txt", type = "output")
for(i in 1:length(fl)){
