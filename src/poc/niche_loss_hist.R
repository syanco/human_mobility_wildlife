...
(out <- do.call("rbind", res_out) %>% 
    arrange(change_prop) %>% 
    mutate(perc = -(1-change_prop)) %>% 
    filter(species != "Numenius americanus")) #hardcode to remove obviously wrong estimate...


# ggplot(out, aes(x = reorder(species, perc), y = perc)) +
...
ggplot(out, aes(x = perc)) +
  geom_histogram(binwidth = 0.01) +
  xlab("Niche Breadth Percent Chnage") +
  ylab("") +
  theme_tufte()
