...
for(i in 1:length(int_modlist_full)){
  ...
  if(int_modlist_full[i] != "NULL"){
    ...
    if(fe["sg_norm:ghm_scale", "Q2.5"] < 0 & 0 < fe["sg_norm:ghm_scale", "Q97.5"]){ # if the interacton effect overalps 0...
      ...
      sg_effects_out[[i]] <- sg_ce[[1]] %>% 
        mutate(species = sp,
               ghm_cond = case_when(effect2__ == "5%" ~ "Low",
                                    effect2__ == "95%" ~ "High"),
               sig_code = case_when(
...
(sg_coef_plot_zoom <- ggplot(sg_es_df) +
    geom_point(aes(x = estimate__, y = reorder(species, estimate__), color = sig_code)) +
    geom_segment(aes(x = lower__, xend = upper__, 
                     y = reorder(species, estimate__), yend = reorder(species, estimate__),
                     color = sig_code)) +
    scale_color_manual(name = "", values = coeff_pal) +
    geom_vline(aes(xintercept = 0), linetype = "dashed") +
    ylab("Species") +
...
(ghm_coef_plot_zoom <- ggplot(ghm_es_df) +
    geom_point(aes(x = exp(estimate__), y = reorder(species, estimate__), color = sig_code)) +
    geom_segment(aes(x = exp(lower__), xend = exp(upper__), 
                     y = reorder(species, estimate__), yend = reorder(species, estimate__),
                     color = sig_code)) +
    scale_color_manual(name = "", values = coeff_pal) +
    geom_vline(aes(xintercept = 0), linetype = "dashed") +
