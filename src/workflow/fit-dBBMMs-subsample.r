...
# Read in output log
log <- read_csv(glue("{.outPF}/dbbmm_log_subsample.csv"))
  ...
  foreach(i = 1:2, .errorhandling = "pass", .inorder = F) %dopar% {
    ...
    if(log %>% 
      ...
      if(nrow(evt_mod) <= 5){
        ...
        write.table(outlog, glue("{.outPF}/dbbmm_log_subsample.csv"), append = T, row.names = F, 
                    col.names = F, sep = ",")
        ...
        if(nrow(evt_mod) > 50000){
          ...
          outlog <- data.frame("species" = scientificname, 
                               ...
                               "produced" = 0, 
                               "out_date" = as.character(Sys.Date()))
          write.table(outlog, glue("{.outPF}/dbbmm_bigmem_log_subsample.csv"), append = T, row.names = F, 
                      col.names = F, sep = ",")
        } else {
          ...
          tryCatch({
            ...
            evt_mv <- move(x=evt_sub$lon, y=evt_sub$lat, time=evt_sub$timestamp, trt = evt_sub$trt,
                           proj=CRS("+proj=longlat"))
            burstid <- factor(evt_sub$wk[1:(n.locs(evt_mv)-1)])
          ...
          tryCatch({
            if(exists("tmp_out")){
              save(tmp_out,
                   file = glue("{.outPF}/dbbmm_subs/dbbmm_{ind[j]}_{yearvec[i]}.rdata")
              )
