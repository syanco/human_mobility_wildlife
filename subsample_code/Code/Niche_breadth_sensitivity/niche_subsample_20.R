  ...
  # for(j in 1:100){
    ...
    tryCatch({
          ...
          if(nrow(evt_tmp_sub) > 0){      
            ...
            write.table(determinant_df, glue("{.outPF}niche_subsampling_20.csv"), append = T, 
            ...
            write.table(tmp_dummy_success, glue("{.outPF}niche_log_20.csv"), append = T, row.names = F, col.names = F, sep = ",")
...
write.table(tmp_dummy_fail, glue("{.outPF}niche_log_20.csv"), append = T, row.names = F, col.names = F, sep = ",")
