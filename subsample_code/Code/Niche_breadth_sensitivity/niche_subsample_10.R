...
if(interactive()){
  ...
  n_sample_folder = subsample_folders[1]
...
#    paste0(n_sample_folder, '/data_subset_anthropause_', individual_week_year$indiv_week_year[i],'_', n_sample, '.csv')
  paste0(n_sample_folder, '/data_subset_anthropause_', tmp_file,'_', n_sample, '.csv')
  )%>%  # Scale  
  ...
  # for(j in 1:100){
    ...
    tryCatch({
          ...
          if(nrow(evt_tmp_sub) > 0){      
            ...
            write.table(determinant_df, glue("{.outPF}niche_subsampling_10.csv"), append = T, 
            ...
            write.table(tmp_dummy_success, glue("{.outPF}niche_log_10.csv"), append = T, row.names = F, col.names = F, sep = ",")
...
write.table(tmp_dummy_fail, glue("{.outPF}niche_log_10.csv"), append = T, row.names = F, col.names = F, sep = ",")
