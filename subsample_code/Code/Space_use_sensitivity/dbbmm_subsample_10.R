...
foreach(i = 1:nrow(individual_week_year), .errorhandling = 'pass', .inorder = F ) %dopar% {
  ...
  if( ! file.exists(glue("{.outPF}/DBBMM_subsampling/dbbmm_10_log.csv"))  ){
  write.table(
    data.frame(
    ...
    , glue("{.outPF}/DBBMM_subsampling/dbbmm_10_log.csv"), append = T, row.names = F,col.names = T, sep = ",") # Log file
  }
  ...
  for(j in 1:20){
    ...
    tryCatch({
      ...
      message(glue("Writing output for individual {individual_animal_id} to file..."))
        write.table(outlog, glue("{.outPF}/DBBMM_subsampling/dbbmm_10_log.csv"), append = T, row.names = F,col.names = F, sep = ",") # Log file
